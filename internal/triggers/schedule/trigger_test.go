...
func createTestConfig() *schedule.Config {
	return &schedule.Config{
		...
		ScheduleType:    "interval", // Default to interval scheduling
		Interval:        10 * time.Second,
		MaxRuns:         0,
		RunImmediately:  true, // Default to immediate execution for tests
		...
		DataTransform: schedule.TransformConfig{
			Enabled:  false,
			Template: "",
		},
...
func TestScheduleTrigger_DataTransform(t *testing.T) {
	...
	config.Interval = 100 * time.Millisecond
	config.MaxRuns = 1
	config.DataTransform = schedule.TransformConfig{
		Enabled:  true,
...
func TestScheduleTrigger_CronExpression(t *testing.T) {
	...
	config.CronSpec = "*/2 * * * * *" // Every 2 seconds
	config.MaxRuns = 2
	config.RunImmediately = false // Cron should follow its schedule
	...
	tests := []struct {
	...
	}{
		{
			...
			config: &schedule.Config{
				...
				EndTime:  &[]time.Time{time.Now().Add(-1 * time.Hour)}[0], // Already expired
			},
			expectError: true,
		},
		{
			...
			config: &schedule.Config{
				...
				Interval: time.Second,
				MaxRuns:  1, // Only 1 run allowed
			},
			expectError: false, // MaxRuns check happens after start, not during start
		},
	}
