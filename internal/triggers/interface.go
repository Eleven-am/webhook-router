...
// OAuth2Trigger is an optional interface for triggers that support OAuth2
type OAuth2Trigger interface {
	Trigger
	SetOAuthManager(manager *oauth2.Manager)
}

// PipelineTrigger is an optional interface for triggers that support pipeline_old processing
type PipelineTrigger interface {
	Trigger
	SetPipelineEngine(engine interface {
...
func (c *BaseTriggerConfig) GetID() string {
	return c.ID
}
...
func (c *BaseTriggerConfig) Validate() error {
	if c.Name == "" {
...
func (m *TriggerManager) RemoveTrigger(id string) error {
...
func (m *TriggerManager) StartTrigger(id string) error {
...
func (m *TriggerManager) StopTrigger(id string) error {
	trigger, exists := m.triggers[id]
	if !exists {
		return ErrTriggerNotFound
	}
...
func (m *TriggerManager) GetTrigger(id string) (Trigger, error) {
	trigger, exists := m.triggers[id]
	if !exists {
...
func (m *TriggerManager) GetAllTriggers() map[string]Trigger {
	return m.triggers
}
