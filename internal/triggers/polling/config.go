...
func (c *Config) validateAuth() error {
	...
	case "oauth2":
		...
		// Either with access_token directly or with oauth2 service reference
		if c.Authentication.Settings["access_token"] == "" && c.Authentication.Settings["oauth2_service_id"] == "" {
			v.Validate(func() error {
				return errors.ConfigError("either access_token or oauth2_service_id is required for oauth2 auth")
			})
		}
	}
