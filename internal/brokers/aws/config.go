...
func (c *Config) Validate() error {
	...
	if c.QueueURL == "" && c.TopicArn == "" {
		v.Validate(func() error {
			return errors.ConfigError("either QueueURL (for SQS) or TopicArn (for SNS) is required")
		})
	}
...
func (c *Config) GetConnectionString() string {
	// Sanitize connection strings to prevent credential exposure in logs
	if c.QueueURL != "" {
		// Extract just the queue name without exposing full URL structure
		return fmt.Sprintf("sqs://%s/***", c.Region)
	}
	if c.TopicArn != "" {
		// Extract just the topic name without exposing full ARN structure
		return fmt.Sprintf("sns://%s/***", c.Region)
	}
