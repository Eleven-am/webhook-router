...
func (s *JSONTransformStage) Configure(config map[string]interface{}) error {
	...
	if s.config.Template != "" {
		tmpl, err := template.New(s.name).Parse(s.config.Template)
		if err != nil {
...
func (s *JSONTransformStage) Validate() error {
	...
	if !validOperations[s.config.Operation] {
		return fmt.Errorf("invalid operation: %s", s.config.Operation)
	}
	
	if s.config.Operation == "template" && s.config.Template == "" {
		return fmt.Errorf("template operation requires template")
	}
	
	return nil
}

func (s *JSONTransformStage) Process(ctx context.Context, data *pipeline.PipelineData) (*pipeline.StageResult, error) {
	start := time.Now()
	...
	default:
		err = fmt.Errorf("unsupported operation: %s", s.config.Operation)
	}
...
func (s *JSONTransformStage) applyTemplate(input map[string]interface{}, data *pipeline.PipelineData) (map[string]interface{}, error) {
	if s.template == nil {
...
func (s *XMLTransformStage) Process(ctx context.Context, data *pipeline.PipelineData) (*pipeline.StageResult, error) {
	start := time.Now()
...
func (s *XMLTransformStage) convertXMLToJSON(data *pipeline.PipelineData, result *pipeline.StageResult, start time.Time) (*pipeline.StageResult, error) {
...
func (s *XMLTransformStage) extractFromXML(data *pipeline.PipelineData, result *pipeline.StageResult, start time.Time) (*pipeline.StageResult, error) {
...
func (s *TemplateStage) Validate() error {
	...
	if !validFormats[s.config.OutputFormat] {
		return fmt.Errorf("invalid output format: %s", s.config.OutputFormat)
	}
	
	return nil
}

func (s *TemplateStage) Process(ctx context.Context, data *pipeline.PipelineData) (*pipeline.StageResult, error) {
	start := time.Now()
