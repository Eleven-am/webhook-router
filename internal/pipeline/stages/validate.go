...
func (s *ValidationStage) Validate() error {
	...
	if len(s.config.Rules) == 0 {
		return fmt.Errorf("validation stage requires at least one rule")
	}
	...
	if s.config.OnFailure != "" && !validFailureModes[s.config.OnFailure] {
		return fmt.Errorf("invalid on_failure mode: %s", s.config.OnFailure)
	}
	
	return nil
}

func (s *ValidationStage) Process(ctx context.Context, data *pipeline.PipelineData) (*pipeline.StageResult, error) {
	start := time.Now()
...
func (s *FilterStage) Validate() error {
	...
	if len(s.config.Conditions) == 0 {
		return fmt.Errorf("filter stage requires at least one condition")
	}
	
	validLogic := map[string]bool{"and": true, "or": true}
	if !validLogic[s.config.Logic] {
		return fmt.Errorf("invalid logic: %s", s.config.Logic)
	}
	
	validActions := map[string]bool{"include": true, "exclude": true}
	if !validActions[s.config.Action] {
		return fmt.Errorf("invalid action: %s", s.config.Action)
	}
	
	return nil
}

func (s *FilterStage) Process(ctx context.Context, data *pipeline.PipelineData) (*pipeline.StageResult, error) {
	start := time.Now()
